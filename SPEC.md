...
| TOML-KEY-3 | a header segment is never a bare key glued to a quoted one: the quote is refused (`header_quote_after_bare_refused`), and a quote after the bracket starts a quoted segment (`header_quote_at_start_opens_segment`) | that every key, dotted key and header segment `parse` reads is named by its characters, escapes decoded |
