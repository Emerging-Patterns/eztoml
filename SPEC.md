...
| TOML-KEY-3 | In a parsed document, a key's name is its characters however it was written: a bare key, a basic or literal string with its escapes decoded, or a segment of a dotted key or a header, so `get` and `at` find it by those characters | Proved | pending | LAWS.bend header_quote_after_bare_refused; LAWS.bend header_quote_at_start_opens_segment; LAWS.bend header_reads_back; LAWS.bend header_new_table_reads_back; LAWS.bend header_implicit_table_reads_back; LAWS.bend aot_header_reads_back; LAWS.bend aot_header_new_array_reads_back; LAWS.bend aot_header_extends_array_reads_back |
...
TOML-KEY-1, TOML-KEY-2, TOML-GET-1, TOML-GET-2, TOML-READ-1 and TOML-STR-2 are proved; every other row is pending. For the pending rows that name laws already:

| Row | Proved so far | Missing |
| :---- | :---- | :---- |
| TOML-RT-1 | for every bare key `k` and every string `s` that `render` writes with no escape and that is shorter than 2^32 characters, the document of the one pair `k` = `s` reads back with no error as the same document (`render_parse_plain_pair`) | every other document: quoted keys, strings with escapes, the other values, arrays, inline tables, tables, arrays of tables and more than one pair. Also the length of a string: `parse` counts a plain string's characters in a U32, so one of 2^32 characters that `render` writes with no escape reads back as the empty string, and the row as worded does not hold for it ([docs/rfc/eztoml-roundtrip-proofs.md](docs/rfc/eztoml-roundtrip-proofs.md), REVIEW-R1) |
...
| TOML-KEY-3 | a header segment is never a bare key glued to a quoted one: the quote is refused (`header_quote_after_bare_refused`), and a quote after the bracket starts a quoted segment (`header_quote_at_start_opens_segment`); a `[table]` header of segments written as `key` writes them (bare, or basic-quoted with its escapes) reads from the start of a line as the table named by the segments' characters: the walk runs along those names and they become the current table (`header_reads_back`), and a new table, or one made on the way to another, is defined under the last name's characters (`header_new_table_reads_back`, `header_implicit_table_reads_back`); an `[[array]]` header reads the same way (`aot_header_reads_back`), and its new element goes under the last name's characters (`aot_header_new_array_reads_back`, `aot_header_extends_array_reads_back`) | that every key, dotted key and header segment `parse` reads is named by its characters, escapes decoded, however it is spelled (other escapes, `\u` and `\U` of any character, literal strings, blank space around dots), and that `get` and `at` then find it |
