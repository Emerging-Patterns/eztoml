...
None is decided yet. The draft proposes these, each its own PR with master's results against the branch's:
...
| move to `main.bend` at the root and `src/` | none (layout) | REVIEW-1 |
...
| Lint first | bolt pin to v1.7.0, `laws` at warn; S003 and S004 fixed; IO defs marked | lint clean except the law rules |
| Layout | `main.bend` at the root, the rest under `src/`, if REVIEW-1 is accepted | the interface is one file |
