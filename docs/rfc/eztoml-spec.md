...
- [x] <!-- REVIEW-14 (resolved): The round-trip design ([eztoml-roundtrip-proofs.md](eztoml-roundtrip-proofs.md)) found that a plain string of 2^32 characters or more does not read back, since the scanner counts it in a U32. Decided, as that doc's REVIEW-R1 recommended: TOML-RT-1, TOML-RT-2, TOML-RT-3 and TOML-STR-1 are stated for texts shorter than 2^32 characters, as ezjson's rows are. No code change. The doc's other items (an exact `canon` equation as the core lemma, induction on the document, no refactor of `main.bend`, no row to Trusted, and the spike law deleted when TOML-RT-1 lands) were accepted as recommended too. -->
...
| TOML-STR-2 | `render` writes a string value as a basic string that escapes exactly `"`, `\` and the controls U+0000 to U+001F and U+007F (with `\b`, `\t`, `\n`, `\f`, `\r` where TOML has them, and otherwise `\u00` and two lowercase hexadecimal digits), and writes every other character as itself | Proved | pending |
...
| move to `main.bend` at the root, lemmas to `src/` (landed) | none (layout) | REVIEW-1 |
| README build step and import hash (landed) | none (docs) | REVIEW-12 |
