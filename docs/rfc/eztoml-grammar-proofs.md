...
- [x] <!-- REVIEW-G11 (resolved): A tab between a date and a time was read as a datetime. toml.abnf allows only `T`, `t` or a space (%x20) there (`time-delim`), and Python's `tomllib` refuses a tab. `parse` read `d = 1979-05-27<TAB>07:32:00` as a local datetime, because `lex.bare` sent every `ch.sp` character (space or tab) to `lex.bare.date`, and `lex.date.digit` wrote the space into the word whatever the character was. WP-V found it while planning the lift: it makes TOML-TIME-1 and TOML-TEXT-1 false as worded. It is not one of REVIEW-G1's five cases; the earlier probes tried two spaces, not a tab. Options: (a) fix the code, so that only U+0020 enters `LDate` and a tab ends the word as any other blank does (the text then reads as a date followed by more text on the line, which is refused); (b) reword TOML-TIME-1 to allow a tab, against TOML v1.0.0 and `tomllib`. Recommend (a). Decided: (a), as recommended. `lex.bare` now passes `ch.space` (U+0020 only) where it passed `ch.sp`; three proof call sites (`gd_bare_steps`, the KEY-3 context's `kg.bgo` step, `sr.ff.bare`) follow the change, and the gate passes with no other change. After the fix, a tab between a date and a time is refused as a value, in an array and in an inline table; a space still joins them; a tab before a comment or a comma still ends a word. -->
...
Three classifier laws sit in LAWS.bend's `# WP-N1` section, each of the form `n1.got.K(word.val(cs), v) == and(abnf.K(cs), g.val(GvK{cs}, v))` for every word and every value: `integer_word_exact` (TOML-NUM-1, with `prose.int_in_range` in the conjunction), `float_word_exact` (TOML-NUM-2) and `boolean_word_exact` (untagged, since no row covers booleans alone). `dec_int_word` stays, per REVIEW-G10. The accept direction uses the spike's method: the scan's states are followed character by character against the recognizer, with `n1.ish` and `n1.fsh` saying which finished scans hold an integer or a float. The refuse direction extends RT-3's pairing: a word the recognizer refuses finishes the scan in a state that holds no value of the kind, and special words and the datetime route are split off first. Dec-int words reuse the spike's `gs.scan` and `dtext` with the range test left open. Hex, octal and binary words are read through a radix lemma for `dec.go` (`n1.Wgo`: the numeral times the radix plus a carry, proved from `Ar.u_div` and `Ar.u_add` with the U32s kept below 256), and `dec.text` is shown canonical and worth what the digits are. The range needs 2^63 as 8 * 16^15, 8^21 and 2^63. Each is built as small-Nat digits (8 times 16 fifteen times, and so on) and checked against `"9223372036854775808"` character by character. Every large value is named behind a unit `uu`, so the checker never expands one. The values of two canonical numerals are compared by length and then digit by digit, which gives both `i64.step` and `String.eq` in terms of values. Each law fails the gate when its proof is replaced by `{==}`. The WP-N1 proofs alone also fail on each of four classifier bugs planted in a scratch `main.bend`: a leading zero accepted (`n1.Z.go`), `0x` taking `g` (`n1.cls.none`), `1e` read as a float (`n1.fin.nflo.go`) and a doubled `_` accepted (`n1.X.us`). Size: 61 lines of law and about 9,700 of proof (floats and booleans about 4,000, integers about 5,700). The gate is about 25 to 27 s, up from about 21 s. Lint: 0 errors. `main.bend` is unchanged, and no word was found where the code and the grammar disagree beyond the five REVIEW-G1 cases. TOML-NUM-1 and TOML-NUM-2 stay pending: WP-V states them over `parse`.

## Update (WP-V landed)

TOML-NUM-1, TOML-NUM-2 and TOML-TIME-1 are proved, in LAWS.bend's and PROOF.bend's `# WP-V` sections, on top of the REVIEW-G11 fix (`516bdab`), which WP-V found while planning the lift.

The laws:

- **The lift.** `bare_word_reads_on`: from every state that waits for a value (WP-C's `vpos`, every field a bare value leaves alone a parameter), a bare word `w` that `word.val` reads as `v`, then any text that ends it, reads on as that text from the state with `v` placed (`bw.put`, which is `take.top`). A bare word (`bw.word`) is a character that starts a bare value, then characters a bare value keeps, where a space is kept only right after a date-shaped buffer and before a digit, which is the `LDate` state. A text ends it (`bw.end`) when it is empty, or starts with a space, a tab, a line feed, a carriage return, `#`, `,`, `]` or `}`, but not with a date's space before a digit. The law is an equation, since a bare value leaves back, qkind, uni and left as they were. `bare_word_refused`: a word `word.val` refuses leaves the document with an error, whatever follows. `bare_word_reads_on_in_array` and `bare_word_refused_in_array` are the same from an array's item, where the first character is held and read again in the value state.
- **The document, both directions.** For every bare key `k` and bare word `w`, `k = w` and a newline parse, with no error, to one pair named `k` holding `v` (read through `bw.got`) exactly when `w` matches the rule and denotes `v`. The four laws are `integer_parses_exactly` (within signed 64 bits), `float_parses_exactly`, `date_time_parses_exactly` (within RFC 3339's ranges) and `boolean_parses_exactly`. They carry WP-N1's and WP-T1's classifiers to `parse`. `word_refused_in_document`: a bare word that matches none of the four rules (`bw.none`) leaves an error.
- **Every matching word is a bare word.** `integer_is_bare_word`, `float_is_bare_word`, `date_time_is_bare_word` and `boolean_is_bare_word`. A datetime with a space between the date and the time is included, so the rows need no premise about how the scan delimits a word.
- **The document, accept form.** `*_parses_as_denoted`, `*_in_array` and `*_in_inline_table`, twelve laws: for every bare key `k` and every word `w` that matches the rule and denotes `v`, `k = w`, `k = [w]` and `k = {v = w}` with a newline parse to exactly `Doc{"", [VPair{k, v}]}`, the array of `v`, or the inline table whose pair `v` holds it.
- **Render's halves.** `float_render`: `render.atom` writes a float as its sign (a minus, or nothing for a plus: `bw.sign`), then its spelling. `date_time_render_upper`: the text `render.atom` writes for every well-formed datetime holds no `t`, no `z` and no space. With `date_time_render` (the text matches the rules within the ranges and denotes the datetime, the fraction's digits included), that means `T` between the date and the time and `Z` for UTC.

The lift and refusal laws and `word_refused_in_document` are tagged with all three rows. The other laws each carry their own row's tag, and the boolean ones are untagged.

How it is proved:

- **The word.** Its characters are pushed by WP-N's `wd.first` and `wd.step`. A date's space is two steps: `vl.sp` enters LDate, and `vl.dig` pushes the digit with the space before it. Together they leave the scan where pushing the space as a character would (`vl.spd`), so one induction (`vl.run`) covers both kinds of word.
- **The end.** Each of the eight ending characters is one step (`vl.delim` takes them apart). The step from the word's buffer equals the fast step from the placed value (`vl.acc`), by WP-C's `cw.held` for a held `#`, `,`, `]` or `}`, `cw.nl` for a line feed, `vl.eh.cr` for a carriage return (through `MCr`), and `vl.self` for a space or a tab, which a placed value skips. The end of the text goes through `read.fin.close` (`vl.nil.b`, `vl.close`). The placed value's state has one of three shapes by the stack, so each of its properties is proved once per shape (`vl.vt.elim`). After a date's space, a character that is no digit is held and read again from the placed value, in whatever mode it opens (`vl.q`, `vl.dacc`).
- **Refusal.** The ending step fails the scanner (`st.err`). `fail_stays` keeps the error in the scan mode and after a carriage return (`vl.bs`, `vl.bcr`), and `bare_word_refusal_says_why` gives a reason that is not empty.
- **Documents.** WP-S's `kv.key2` brings `k = ` to the value state. The scalar placed, then the newline, `]` or `}`, gives the document by evaluation (`vl.doc.*`), once `word.val` is known to give only scalars (`vl.sc.word`, a case split on its routes, as `gd_word`'s). `vl.eq.*` turn the classifiers' equalities (`sign.eq`, `String.eq`, `bit.eq`, `when.eq`) into equality of values.
- **Shapes.** A character test that fails the twelve characters that do not start a bare value passes only characters that do (`vl.fq`, by `Eq.refute_char`). One lemma covers DIGIT, HEXDIG, the radix digits, the signs, `_`, `.`, `e` and the letters. Integers and floats follow their recognizers (`vl.qs.*`, with `vl.qs.cut` for the cut at the first `.` or `e`). The date-time rules are handled as `t1.e` handles an exponent mark: a character that starts no bare value stays in what every rule leaves (`vl.z.*`), except time-delim's space. When time-delim is the space, full-date is ten steps (`vl.runr`), and `vl.walk` takes the word apart into the ten characters and the rest. The ten are the date `date.shaped` checks (`vl.deq`), and a partial-time starts with a digit (`vl.pt.d1`), so the scan keeps the space.

Rows against the laws, clause by clause:

- **TOML-NUM-1.** "Read exactly when it matches `integer` and lies within signed 64 bits": `integer_parses_exactly`, `integer_is_bare_word` and `word_refused_in_document`, over the lift. "Reads to its sign and its value's decimal digits, with no leading zero unless the value is 0": `g.val`'s denotation, in `integer_parses_exactly` and `integer_parses_as_denoted`.
- **TOML-NUM-2.** "Read exactly when it matches `float`": `float_parses_exactly` and `float_is_bare_word`. "Reads to its sign and its spelling with the sign and every `_` removed": `g.val`'s denotation, in the same laws. "`render` writes the sign (`-` only) and that spelling": `float_render`, with `flo_word`.
- **TOML-TIME-1.** "Read exactly when it matches one of the four date-time rules with RFC 3339's ranges": `date_time_parses_exactly` and `date_time_is_bare_word`, with a space kept through LDate. "`render` writes it with `T` and `Z` in upper case, keeping the fraction's digits": `date_time_render_upper` with `date_time_render`.

Every clause is covered, and no row was reworded.

Findings: REVIEW-G11, a tab between a date and a time read as a datetime. It was fixed in `516bdab` before these proofs. No other disagreement between the code and the grammar was found. `main.bend` is unchanged from `516bdab`.

Size: 529 lines of law and about 4,160 of proof (planned 1,000, estimated 2,500 to 4,000). The lift is about 1,170 lines, the documents about 700, and the shapes about 1,800, of which the date-time rules are about 900. The gate is about 22.1 s, against about 20.7 s at `516bdab` (three alternating runs each, on the same machine). Lint: 0 errors, and 140 warnings, as at `516bdab`.

Break checks: replacing each of the 27 laws' proofs by `{==}` fails the gate. Five bugs were planted in a scratch copy of `main.bend`. Each fails the full gate first on an older proof. With the older proofs stubbed (`isolate_mutant.py`), four of them fail a WP-V proof:

| Planted bug | Fails first (stubbed) | WP-V proof that fails |
| :---- | :---- | :---- |
| `#` does not end a word | `wd.first`, `cv.xb.w` | `vl.rf.hash` |
| a date's space never kept | `gd.g.bdgo`, `kg.bdgo` | `vl.sp` |
| `render` writes `+` for a plus sign | `int.plus`, `int.chars.nz`, `flo.plus`, `flo.chars.sg` | `float_render` |
| a tab joins a date and a time (REVIEW-G11 undone) | `gd_bare_steps`, `kk.on`, `sr.ff.bare` | `vl.rf.tab` |
| `render` writes `t` for an offset datetime | `when.eq` | none of its own |

The fifth is caught by WP-N's `when.eq`, which equates `when.text` with its tokens. `date_time_render_upper` rests on that lemma, as WP-T1's `t1.days` rests on `feb.same`.
...
`text_reads_as_replay` is proved, as the chunk 1 note states it (with `+` on the hypotheses), and tagged TOML-TEXT-1 and TOML-TEXT-2. For every derivation with `g.ok(d)` whose text is shorter than 2^32 characters, `own(parse(g.text(d))) == replay(d)`. That covers keyvals of every spelling (dotted or not, at the root, after a header, inside inline tables), values of every kind, and arrays and inline tables nested to any depth. It also covers ws-comment-newline anywhere an array allows it, trailing commas, headers and blank lines in any order, LF and CRLF, and a walk or a word that sets an error. The chunk's first commit proved the case with no arrays or inline tables (`scalars_parse_as_replay`); the second replaces it with the whole law. `replay` and `main.bend` are unchanged.

How it is proved:

- **The line invariant is own-level (`aq.K`).** After an expression, the rest of the text, read from the state its `[ comment ]` leaves, gives a document that `own` reads as the lines' replay from `own.rows(rw)`. An error the scanner keeps is the replay's, read out after sealing, since `own` commutes with `rows.seal` (`aq.err`, from WP-C's `cv.osl`).
- **`own` and the walk (`ow.*`).** The walk never reads a string's characters, so `own.rows` of what a walk returns is what the walk returns on `own.rows` of what it was given (`ow.walk`), with a `JPut`'s value read out as in a pair (`ow.v`). The lookups (`rows.find`, `rows.look`, `look.last`), `rows.repl` and the frames commute with `own` the same way. `inl.seal` commutes with it as well (`aq.oinl`).
- **Readers over a motive (`kx.*`).** KEY-3 states its key and header readers with the rest equal to one document for every back, uni and left the key leaves. A document with values in it is known only as `own` reads it. So the readers are restated over a motive `PP` of the document, with the same proofs and each equation to the goal read as `PP` of its side. The header step of chunk 1 goes through them again at the `own` level (`aq.hcore`, `aq.table`, `aq.xtb`).
- **Keyval lines (`aq.xkv`, `aq.kvl`).** `kx.keys` reads the key and `=` from the start of a line, with the segments pushed onto parts. The blank space after `=` is passed over in LVal (`ap.ws`). The value is placed by `take.top`, and its walk from the root is the replay's (`ow.walk`, `k3.path`). The rest of the line reads as after a header (`aq.hl`).
- **Values, one induction (`aq.rd`).** It proves three claims of every value node: as a value (`aq.VT`, after `=` or as an array's item), as an array's items (`aq.IT`, from LArr with the array's frame on the stack), and as an inline table's keyvals (`aq.KT`, from LInl or LInlK). The claims a node's rule rules out hold because `g.ok` refuses the node there (`aq.nov`, `aq.noi`, `aq.nok`). Each claim is taken over a continuation: the value placed (`aq.vk`), the frame's items (`aq.ik`), or its rows (`aq.rk`), with items and rows that `own` reads as the replay's. An error the node sets gives the document of the error and the rows. The value continuation gives both readings of the value, as a pair's (`ow.v`) for the walks and as a list's item (`cv.own1`) for the arrays.
- **The scalar values.** Strings come through WP-S1's `string_reads_as_denoted`, or its `_in_array` form for an item. Bare words come through WP-V's `bare_word_reads_on`, and an item's word is read as after `=` once its first character is held (`vl.arr.eq`). A word that `word.val` refuses reads to the document of its reason and the rows (`wr.lift`: WP-V's `vl.blift` with `error_stays` in place of `fail_stays`). `replay.got` gives that same error, so the proof needs no fact about which words `word.val` reads. The chunk 1 plan expected to show that every word `g.ok` allows is read, through WP-N1's classifiers and radix lemmas; that is not needed. What follows a value is a follower (`aq.fol`: blank space, then the end, `#`, a newline, `,`, `]` or `}`). A follower ends a bare word and starts no string.
- **Arrays and inline tables (`aq.aopen`, `aq.aclose`, `aq.iopen`, `aq.iclose`, `aq.wcn`).** `[` and `{` open a frame, and from an item's state they read as after `=` with the back state LVal (checked by computation). ws-comment-newline is passed over in LArr and LArrC, with chunk 1's comment and newline readers (`aq.wcn`). `,` moves on to LArr or LInlK. `]` places the items in order and `}` places the rows sealed, through `take.top` with nothing held (`aq.once`). An inline table's keyval walks into the frame's rows (`take.inl`), and a walk the rows refuse leaves its error, which stays (`aq.ikw`).
- **Errors in the replay (`aq.gobad`, `aq.stays`).** A value node read from an error keeps it at any depth, since every walk inside it starts from the error and fails. So every expression replayed from an error keeps it, the rows and the definitions, keyvals included. That is chunk 1's `ap.stays` without its `ra.nokv` condition.
- **The document (`aq.go`, `aq.nb.expr`).** `aq.go` is chunk 1's `ap.go` at the own level. A keyval's text starts with blank space and a key, whose first character is a bare-key character, `"` or `'`, never U+FEFF (`aq.nb.*`). So `ap.lead` applies to every derivation.

Findings: none. The code does what the grammar says everywhere the law reaches: keyvals of every spelling, values of every kind, and arrays and inline tables nested to any depth, with every separator and blank space the grammar allows. `main.bend` is unchanged.

Size: 25 lines of law (the law and a header comment) and 5,989 of proof. By part: KEY-3's readers over a motive 790, `own` and the walk 790, a refused word's document 317, followers, scalar readers and continuations about 740, errors in the replay 262, headers at the own level 272, the array and inline-table steps 460, the value induction about 1,720, and keyval lines and the document about 670. That puts WP-A at about 12,000 lines, at the low end of the plan's estimate. The gate is about 23 to 26 s, against about 22 to 24 s at `bd5d4ec` on the same machine. Lint on a clean export of HEAD: 0 errors, and 140 warnings, as before.

Break checks: replacing `text_reads_as_replay`'s proof by `{==}` fails the gate, and so do `error_stays` and `headers_parse_as_replay`, rerun. Six bugs were planted in a scratch copy of `main.bend`. Each fails the full gate first on an older proof. With those stubbed (`isolate_mutant.py`, naming only this chunk's proofs), each fails a chunk 2 proof:

| Planted bug | Fails first (stubbed) | Chunk 2 proof that fails |
| :---- | :---- | :---- |
| a root keyval's refused walk is forgotten (`take.root`), so a key defined twice is accepted | `kk.troot`, `vl.vt.root`, `fz.top` | `aq.kvw` |
| a root keyval's dotted key is walked unreversed (`take.top`) | `vl.vt.elim`, `vl.eh.cr`, `fz.top` | `aq.once` |
| an array's items are placed newest first (`arr.top`) | `gd.g.arrtop`, `kk.arrtop`, `fz.atop` | `aq.aclose` |
| an inline table's refused walk is forgotten (`take.inl`) | `kk.tinl`, `vl.vt.inl`, `fz.top` | `aq.ikw` |
| a comma in an array waits for another comma (`lex.arrc.comma`) | `cv.after`, `gd.g.accomma`, `kg.accomma` | `aq.itemk` |
