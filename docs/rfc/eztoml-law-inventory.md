...
| Four | in progress | The design doc [eztoml-grammar-proofs.md](eztoml-grammar-proofs.md) plans TOML-TEXT-1, TEXT-2, STR-1, NUM-1, NUM-2 and TIME-1 as eight work packages over one law-side object: toml.abnf transcribed as a derivation type with Bool recognizers at its leaves (about 52,000 to 84,000 lines of proof, calibrated against the round trip's record, and a gate of 30 to 45 s). Its spike transcribed `dec-int` rule by rule (`abnf.dec_int`) and proved `dec_int_word` (tagged TOML-NUM-1): every word matching it within signed 64 bits, `+` and `_` included, is read as its sign and its digits without underscores (792 lines of proof, 143 of law); a planted bug reading `_` as a 0 digit passed the gate before and fails it now. The gate went from about 17 s to about 18 s. Probing found TOML-TEXT-1 and TOML-TEXT-2 false as worded where toml.abnf and the prose disagree (integers out of range, dates out of range, `\u` of a surrogate, U+007F in a comment, a leading byte-order mark), which the doc's REVIEW-G1 and REVIEW-G2 put to the maintainer. No change to `main.bend` |
