...
| Three | in progress | WP0 landed: the quoted-segment lemmas (`qs.*`) read key and header segments alike, `Eq.ctl.all` replaces the copied control enumerations, and seven U32 and word lemmas moved to `src/eq.bend`. The gate takes 7.4 s. WP-W landed: twelve contract laws for `tree.walk` (a put, a table header, an array-of-tables header, a failed walk, `rows.seal`, and reading back a table the walk set), four of them tagged TOML-TEXT-2; each fails the gate when its proof is replaced by `{==}`, and planted walk bugs (a put at the tail, `[[a]]` starting empty, an error cleared, a seal that does not recurse) each fail on a new law. The gate takes 7.5 s. WP-H landed: `[..]` and `[[..]]` headers with bare or basic-quoted segments read back from the line-start state, six laws tagged TOML-KEY-3 (1,190 lines of proof); each top-level proof fails when replaced by `{==}`, and eight planted header bugs (a segment dropped, a dot that pushes nothing, `]]` read as `]`, `[[` not marking an array of tables, the walk job swapped) each fail the gate. The gate takes 8.1 s. WP-S landed: every rendered string reads back as a root pair's value, an array item and an inline table's value (three laws tagged TOML-STR-1, 2,668 lines of proof); twenty proofs each fail when replaced by `{==}`, and six planted reader bugs each fail on a WP-S proof, one of them (a copied span dropping a character) caught by nothing before. The gate took 12.3 s; computing each control's escape once (`ctl.esc.lit`, `ctl.esc.all`) brought it back to about 7 s. WP-N landed: booleans, integers, floats and datetimes read back as themselves, by classifier laws tagged TOML-NUM-1, TOML-NUM-2 and TOML-TIME-1 and by `scalar_reads_back` in the scanner's premise form; `src/arith.bend` ports ezjson's U32 arithmetic for the year. Every new law fails when replaced by `{==}`, and nine planted renderer and reader bugs (`True`, a lost minus sign, a digit dropped, month and day swapped, a fraction's dot dropped) each fail the gate. The gate takes about 15 s. WP-K3 landed ten laws tagged TOML-KEY-3 (literal keys, dotted keys with blank space, the put a dotted key makes and `at` finding it, header segments spelled any way); the row stays pending for other basic-string spellings, inline-table keys, other values and whole documents. Each law fails when replaced by `{==}`, and six planted key bugs each fail on a new proof. The gate takes about 16 s. WP-C landed: every well-formed value `render` writes reads back in a value position, arrays and inline tables nested to any depth (`value_reads_back`, `pass_value`), and the partial law `render_parse_value_pair` tagged TOML-RT-1 (a one-pair document with any non-table value reads back `same` as itself). Each law fails when replaced by `{==}` or a wrong term, and four planted reader bugs (a space after `,` refused, array items reversed, a nested closer clearing the stack, an inline table not sealed) each fail on a WP-C proof. The gate takes about 17 s. The WP-R3 spike landed eight laws tagged TOML-RT-3 over every scanner state, not only rendered text: the invariant `good` holds at the start, is kept by `tree.walk`, by every header and key state, by 18 value states and by placing a value, and a good end state with no error gives a `wf` document (5,393 lines of proof; the gate is unchanged). Each proof fails when replaced by `{==}`, and four planted walk and header bugs each fail on a new proof. What is left (the classifier's values, bare words, the fast dispatch, span modes and the induction over the text) is estimated at 4,000 to 6,000 more lines, so about 10,000 in all against 3,000 to 5,000 planned; the row stays Proved. WP-D landed and TOML-RT-1 is proved: `render_parse` (the row), `render_parse_canon` (the exact lemma, `own(parse(render(d))) == canon(d)`) and `canon_same`, all tagged TOML-RT-1, by induction over the key pass and then the table pass (8,179 lines of proof, 105 of law); the spike's partial laws `render_parse_plain_pair` and `render_parse_value_pair` are deleted. Every top-level proof fails when replaced by `{==}`, and four planted renderer bugs (pairs after tables, a header's pairs dropped, array-of-tables elements reversed, a scalar pair skipped by the key pass) each fail the gate, the first three on WP-D proofs once older laws are stubbed. No change to `main.bend`. The gate takes about 18 s. WP-R3 landed and TOML-RT-3 is proved: `parse_wf` (the row) with `gd_read`, the induction over the text, and seven more laws for the bare word's values and states, the fast step and held character, and the span modes (`gd_word`, `gd_bare_steps`, `gd_fast_step`, `gd_held`, `gd_span_wf`, `gd_mode_step`, `gd_next`), all tagged TOML-RT-3 (5,433 lines of proof beyond the spike, 10,826 in all). Every new law fails when replaced by `{==}`, and four planted bugs (a digit after a leading zero, month 13, a span counting two per character, a closed span one character too long) each fail on a WP-R3 proof. No change to `main.bend`. The gate takes about 18.5 s. WP-R2 landed and TOML-RT-2 is proved: `parse_render_parse` (the row), from `render_parse` and `parse_wf` for the first half, and for the second `render_parse_canon` read through `render`: `render` writes a document read out as it writes it, and writes canon of a well-formed document as it writes the document (1,226 lines of proof, 20 of law, against 300 planned). Every new proof fails when replaced by `{==}`, and three planted renderer bugs (a span written raw, an array-of-tables element under the wrong path, the key pass reversed) each fail on a WP-R2 proof once the older proofs they break are stubbed. No change to `main.bend`. The gate takes about 19 s. TOML-KEY-3 was reworded (REVIEW-R7: `at` is claimed only off arrays, and the row has the 2^32 bound), and three laws tagged TOML-KEY-3 landed: `basic_key_reads_back` (a basic key of any pieces, `\u` and `\U` of any scalar value in either case among them, read by a symbolic hex reader), `key_reads_back_anywhere` (a dotted key spelled any way, from the start of a line or an inline table's key position, with the stack, `uni` and `left` free) and `header_reads_back_anywhere` (a header spelled any way, from any `uni` and `left`); 3,185 lines of proof. Each fails when replaced by `{==}`, and four planted key-reading bugs each fail on a new proof. The gate takes about 22 s on this machine. Left in row Three: TOML-KEY-3, for any value and whole documents TOML-KEY-3 proved: `key_found_after_equals`, `key_found_anywhere`, `key_found_in_document`, `header_found_anywhere`, `key_found_by_at`, `header_found_by_at` and `bare_word_refusal_says_why`, by the scan induction with the invariant `kk.K` (see the roundtrip-proofs note). |

| Four | in progress | The design doc [eztoml-grammar-proofs.md](eztoml-grammar-proofs.md) plans TOML-TEXT-1, TEXT-2, STR-1, NUM-1, NUM-2 and TIME-1 as eight work packages over one law-side object: toml.abnf transcribed as a derivation type with Bool recognizers at its leaves (about 52,000 to 84,000 lines of proof, calibrated against the round trip's record, and a gate of 30 to 45 s). Its spike transcribed `dec-int` rule by rule (`abnf.dec_int`) and proved `dec_int_word` (tagged TOML-NUM-1): every word matching it within signed 64 bits, `+` and `_` included, is read as its sign and its digits without underscores (792 lines of proof, 143 of law); a planted bug reading `_` as a 0 digit passed the gate before and fails it now. The gate went from about 17 s to about 18 s. Probing found TOML-TEXT-1 and TOML-TEXT-2 false as worded where toml.abnf and the prose disagree (integers out of range, dates out of range, `\u` of a surrogate, U+007F in a comment, a leading byte-order mark), which the doc's REVIEW-G1 and REVIEW-G2 put to the maintainer; both rows were reworded. Since then: WP-G transcribed toml.abnf as the grammar relation; WP-T1, WP-N1 and WP-S1 made the datetime, number, boolean and string readers exact against it; WP-V lifted the bare words over `parse` from every value state. TOML-STR-1, TOML-NUM-1, TOML-NUM-2 and TOML-TIME-1 are proved. WP-V's planning found one bug, a tab between a date and a time read as a datetime (REVIEW-G11), fixed in `main.bend` (only U+0020 may join them). The WP-R spike sized TOML-TEXT-1's refusal direction. Left: WP-A and WP-D2 (TOML-TEXT-2), then WP-R (TOML-TEXT-1) |
